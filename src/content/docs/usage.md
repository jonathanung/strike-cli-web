...
While a **root** turn is running you can redirect it without a hard interrupt:

| Mechanism | Behavior |
|---|---|
| Protocol / RPC / SDK `steer` op | Applies at the next safe provider request boundary |
| Queued `user.input` | Waits until the turn ends (does not mid-turn redirect) |
| `interrupt` | Cancels the in-flight turn |

Durable `turn.steered` events record whether the steer applied at a boundary,
restarted after cancel, or fell back to the queue. Child sessions still use
`task` action=`message` for parent→child steer only.

## Headless approval control

`strike exec --approval-control <unix-socket|fifo>` opens an NDJSON channel for
per-request permission and question replies (once/reject; durable always/project
requires explicit `durable:true`). Timeouts, disconnects, and malformed replies
**fail closed**. Without the flag, exec keeps legacy auto-reject behavior.
Request payloads are secret-redacted.

Also useful with headless runs:

```sh
strike exec --max-cost 1.50 "…"
strike --turn-timeout 30m   # or off
```
